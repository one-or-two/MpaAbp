...
namespace MpaAbp.Tests
{
    public abstract class MpaAbpTestBase : AbpIntegratedTestBase<MpaAbpTestModule>
    {
        private DbConnection _hostDb;
        private Dictionary<int, DbConnection> _tenantDbs; //only used for db per tenant architecture

        protected MpaAbpTestBase()
        {
        ...
        protected void UsingDbContext(Action<MpaAbpDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MpaAbpDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<MpaAbpDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<MpaAbpDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MpaAbpDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MpaAbpDbContext>())
                {
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MpaAbpDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MpaAbpDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<MpaAbpDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<MpaAbpDbContext, Task<T>> func)
        {
            T result;
